...
encoder output through the decoder in `tests/dk_codec_test.c`, and
...
- `tests/dk_codec_test.c` — direct codec exerciser. Decoder covers
...
- `tests/dk.bats` — end-to-end CLI tests. STORED open / extract,
  ...
  detection.
- `tests/c_api_test.c` — public API round-trip: `pakka_create` for
...
`tests/fixtures/dk/`: synthetic input bytes (`inputs/*`, generated by
`inputs/generate.py`) are packed into `user.pak` by `dkpak`;
`tests/dk.bats` consumes the result and `cmp`s every extracted
payload against the committed source. The case hard-fails when
`user.pak` is missing so accidental deletion can't silently drop
the coverage.
